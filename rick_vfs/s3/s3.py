...
class MinioBucket(VfsVolume):
    ...
    def enable_versioning(self, mfa_delete=False):
        mfa_delete = 'Disabled' if not mfa_delete else 'Enabled'
        try:
...
class MinioVfs(VfsContainer):
    ...
    def rmdir(self, directory_name, **kwargs) -> Any:
        ...
        try:
            ...
            if info is not None and info.is_dir:
                ...
                raise VfsError(f"rmdir(): '{directory_name}' is not a directory")
        except S3Error as e:
            raise VfsError(e)
    ...
    def ls(self, path=Path('/'), **kwargs) -> List[MinioObjectInfo]:
        ...
        try:
            path = str(path)
            result.extend(
                MinioObjectInfo(item)
                for item in self.client.list_objects(
                    self.bucket_name, prefix=path, **kwargs
                )
            )
            return result
        except S3Error as e:
